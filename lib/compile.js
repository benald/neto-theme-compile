...
// Check if master source already exists instead of nuking and cloning every time
...
class NetoThemeCompiler {
    ...
    cloneRepo() {
        ...
        return fs.emptyDir(this.options.repoPath)
            .then(() => {
                return git()
                    .silent(true)
                    .clone(this.options.repo, this.options.repoPath);
            })
            .then(() => { this.log(`Done!`); });
    }
    ...
    compileFull() {
        ...
        return fs.pathExists(this.options.srcPath)
            ...
            .then(() => { return fs.copy(this.options.repoPath + 'src/', this.options.distPath); })
            .then(() => { return fs.copy(this.options.srcPath, this.options.distPath); })
            .then(() => { return fs.remove(this.options.distPath + '/.git'); })
            .then(() => { this.log(`Done!`); })
    }
    ...
    compileFile(relative) {
        ...
        return fs.copy(this.options.srcPath + relative, this.options.distPath + relative)
            ...
            .catch(err => {
                ...
                return fs.copy(this.options.repoPath + 'src/' + relative, this.options.distPath +
                        relative)
                    .then(() => {
